			...
			<div class="tablepage_box">
				...
				<div class="pageBox" :style="{marginTop: '25px',paddingBottom: '35px'}">
					...
					<Page :total="yhlbmktablePageData.total" :page-size="yhlbmktablePageData.pageSize" :current="yhlbmktablePageData.currentPage"
					 show-elevator ref="pages" @on-change="yhlbmkPageChange"></Page>
	...
	export default {
		...
		methods: {
			...
			yhlbmkGetList(currentPage, isSearch) {
				...
				console.log(params)
				axios.get('/api/manage/wxUser/getList', {
						params
					})
					.then((response) => {
			...
			yhlbmkSearch(currentPage) {
				if (this.yhlbmkIpVal) {
					...
					// let postData = this.$qs.stringify(params);
					axios.get('/api/auction/user/sys/getUserByPhone', {
							params
						})
						.then((response) => {
							var res = response.data;
			...
			changeUser(id) {
				if (confirm('是否确认变更类型？') == true) {
					...
					// let postData = this.$qs.stringify(params);
					axios
						.get('/api/manage/tLevel/setPromotersInfo', {
							params
						})
						.then(response => {
							if (response.data.code == 200) {
